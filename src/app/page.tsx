    ...
    <div className="relative isolate min-h-svh overflow-x-clip bg-background text-foreground">
      ...
      <main>
        ...
        <section id="produk" className="relative overflow-hidden">
          ...
          <div className="mx-auto max-w-6xl px-6 pt-16 pb-12 sm:pt-24">
            <div className="mx-auto max-w-3xl text-center">
              {/* label — quick & light, sets the stage */}
              <Reveal as="span" trigger="mount" delay={0} duration={0.45} y={8} className="inline-flex items-center gap-2 rounded-full border border-border bg-muted/50 px-3 py-1 font-heading text-[11px] uppercase tracking-wider text-muted-foreground">
                <span className="size-1.5 rounded-full bg-primary" />
                AI Helpdesk untuk SIMRS &amp; Rumah Sakit
              </Reveal>

              {/* headline — the anchor: slow, weighty, settles into focus */}
              <Reveal as="h1" trigger="mount" delay={0.16} duration={0.95} y={36} blur={10} className="mx-auto mt-6 max-w-3xl text-balance text-4xl font-bold leading-[1.1] tracking-tight sm:text-5xl">
                Helpdesk cerdas untuk operasional rumah sakit.
              </Reveal>
              {/* subhead — trails the headline */}
              <Reveal as="p" trigger="mount" delay={0.52} duration={0.6} y={20} className="mx-auto mt-4 max-w-2xl text-balance text-xl font-semibold leading-snug text-foreground/85 sm:text-2xl">
                AI menjawab, SLA terpantau, semua modul dalam satu inbox.
              </Reveal>

              {/* supporting copy — lighter, follows close behind */}
              <Reveal as="p" trigger="mount" delay={0.66} duration={0.55} y={14} className="mx-auto mt-6 max-w-xl text-pretty text-base text-muted-foreground">
              ...
              {/* primary action — punches in after a deliberate gap */}
              <Reveal trigger="mount" delay={0.9} duration={0.5} y={18} className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                <Button asChild size="lg" className="h-12 px-6 text-base font-semibold transition-transform duration-200 hover:scale-[1.03] active:scale-95">
                  <Link href="/sign-in">
                    Coba SalamDesk
                    <RiArrowRightLine className="size-4" />
                  </Link>
                </Button>
                <Button asChild size="lg" variant="outline" className="h-12 border-border bg-background/80 px-6 text-base text-foreground shadow-sm backdrop-blur-sm transition-transform duration-200 hover:scale-[1.03] hover:bg-background active:scale-95">
                  <Link href="#kontak">Jadwalkan Demo</Link>
                </Button>
              </Reveal>

              {/* trust row — subtle, last to arrive */}
              <Reveal as="p" trigger="mount" delay={1.08} duration={0.45} y={6} className="mt-6 flex flex-wrap items-center justify-center gap-x-5 gap-y-2 text-xs text-muted-foreground">
            ...
            {/* Product showcase — the finale */}
            <Reveal trigger="mount" delay={1.25} y={40} duration={0.9} blur={6} className="relative mx-auto mt-16 max-w-8xl">
        ...
        <section id="fitur" className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
          <Reveal as="div" y={24} duration={0.75} className="mx-auto max-w-2xl text-center">
          ...
          <RevealStagger delay={0.25} stagger={0.07} className="mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((f) => (
              <RevealItem
                key={f.title}
                lift
        ...
        <section id="cara-kerja" className="border-y border-border bg-muted/30">
          <div className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
            ...
            <RevealStagger delay={0.3} stagger={0.08} className="mt-14 grid gap-8 md:grid-cols-3">
              {STEPS.map((s) => (
                <RevealItem key={s.n} y={24} duration={0.6} className="relative">
        ...
        <section id="kontak" className="mx-auto max-w-6xl px-6 py-20 sm:py-28">
          <Reveal as="div" y={28} duration={0.7} className="relative overflow-hidden rounded-3xl border border-border bg-zinc-950 px-8 py-16 text-center text-white sm:px-16">
            ...
            <RevealStagger delay={0.2} stagger={0.08} className="relative mx-auto max-w-2xl">
              <RevealItem as="h2" y={24} duration={0.7} className="text-3xl font-bold tracking-tight sm:text-4xl">
                Siap mengubah cara tim Anda menangani tiket?
              </RevealItem>
              <RevealItem as="p" y={16} duration={0.5} className="mt-4 text-zinc-300">
                Masuk untuk mulai, atau jadwalkan demo bersama tim kami untuk melihat
                SalamDesk berjalan di alur kerja rumah sakit Anda.
              </RevealItem>
              <RevealItem y={14} duration={0.5} className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                <Button asChild size="lg" className="h-12 px-6 text-base font-semibold transition-transform duration-200 hover:scale-[1.03] active:scale-95">
                  <Link href="/sign-in">
                    Coba SalamDesk
                    <RiArrowRightLine className="size-4" />
                ...
                <Button
                  ...
                  size="lg"
                  variant="outline"
                  className="h-12 border-white/20 bg-white/10 px-6 text-base text-white transition-transform duration-200 hover:scale-[1.03] hover:bg-white/20 hover:text-white active:scale-95"
                >
                  <Link href="/sign-in">Jadwalkan Demo</Link>
                </Button>
              </RevealItem>
              <RevealItem as="ul" className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-2 text-sm text-zinc-300">
